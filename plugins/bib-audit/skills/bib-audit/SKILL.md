...
- `BIB_AUDIT_MAILTO=you@example.org` joins Crossref's polite pool — anonymous clients get throttled harder on a few hundred entries.
- `S2_API_KEY=…` authenticates Semantic Scholar. Optional, but worth having: unauthenticated S2 returned `500 Internal Server Error` for an entire day during development, and S2 is the only source that resolves a paper carrying no DOI and no arXiv ID (AdamW's S2 record has only MAG/DBLP/CorpusId). Request one at <https://www.semanticscholar.org/product/api>. The key is sent to `semanticscholar.org` and nowhere else.
